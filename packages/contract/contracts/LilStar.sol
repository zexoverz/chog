...
import "@turbo-eth/solbase-sol/src/tokens/ERC721/ERC721.sol";
...
contract LilStar is ERC2981, ERC721, Ownable, OperatorFilterer {
        ...
        returns (uint256[] memory)
    {
        ...
        if (msg.sender != info.blindBoxAddress) {
            revert InvalidRedeemer();
        }

        uint256 amount = blindBoxIds.length;
        uint256[] memory tokenIds = new uint256[](amount);

        // Assume data has already been validated by the BlindBox contract
        for (uint256 i; i < amount; ) {
            uint256 blindBoxId = blindBoxIds[i];

            uint256 tokenId = _useRandomAvailableTokenId();
            // Don't need safeMint, as the calling address has a BlindBox in it already
            _mint(to, tokenId);
    ...
    // On-chain randomness tradeoffs are acceptable here as it's only used for the LilStar's id number itself, not the resulting LilStar's metadata (which is determined by the source BlindBox).
    ...
    function setBlindBoxAddress(address contractAddress) external onlyOwner {
        redeemInfo = RedeemInfo(redeemInfo.redeemBlindBoxOpen, contractAddress);
    }

    function setRedeemBlindBoxState(bool _redeemBlindBoxOpen) external onlyOwner {
        address blindBoxAddress = redeemInfo.blindBoxAddress;
        if (blindBoxAddress == address(0)) {
