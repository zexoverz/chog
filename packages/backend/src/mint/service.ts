...
export class MintService {
  ...
  getCurrentPhase(): MintPhase {
    ...
    if (this.starlistStartTime > 0 && now >= this.starlistStartTime && now < this.starlistEndTime) {
      return MINT_CONFIG.PHASES.STARLIST;
    }

    if (this.fcfsStartTime > 0 && now >= this.fcfsStartTime && now < this.fcfsEndTime) {
  ...
  setPhaseTimes(times: {
    ...
    if (times.starlistStart !== undefined) this.starlistStartTime = times.starlistStart;
    if (times.starlistEnd !== undefined) this.starlistEndTime = times.starlistEnd;
   ...
   */
  async generateSignature(address: Address, amount: number): Promise<SignatureResult> {
    const normalizedAddress = getAddress(address);
