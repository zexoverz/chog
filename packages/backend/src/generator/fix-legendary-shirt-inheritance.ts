...
const OUTPUT_PATH = join(import.meta.dir, "../../output/common_fix_legendary_shirts");
...
const REPLACEMENT_SHIRTS: Record<string, string[]> = {
	...
	bearBunnyFox: ["Chogstar", "Astronaut", "Black Hoodie", "Monad", "Self Love Club"],
};

const LAYER_ORDER = [
	"background", "base", "clothes", "necklaces", "mouth", "eyes",
	"eyeglasses", "head_acc", "hand", "hand_accessories", "side_hand", "side_hand_accessories",
];
...
async function regenerateImage(metadata: any, character: CharacterType, outputImagePath: string) {
	...
	for (const attr of metadata.attributes) {
		...
		const legendaryFolderMap: Record<string, string[]> = {
			...
			"Shirt": ["clothes"],
			"Eyes": ["eyes"],
			"Hand Accessories": ["hand_accessories_gold", "side_hand_accessories_gold"],
		};
		...
		if (layerPath) {
			...
			const zIndex = LAYER_ORDER.indexOf(
				...
				layerName === "hand_accessories" ? "hand_accessories" : // Render BEFORE side_hand (hand holds the accessory)
				layerName
			);
	...
	if (layers.length > 1) {
		const overlays = await Promise.all(
			layers.slice(1).map(async (layer) => ({ input: await sharp(layer.path).toBuffer() }))
		);
...
async function fixLegendaryShirtInheritance() {
	...
	const legendaryShirtNFTs: Record<string, { tokenId: string; character: CharacterType }[]> = {};
	...
	for (const char of characters) {
		...
		for (const file of files) {
			...
			if (shirtAttr) {
				const matchedShirt = LEGENDARY_SHIRTS.find(l => l.toLowerCase() === shirtAttr.value.toLowerCase());
				if (matchedShirt) {
	...
	const changes: { tokenId: string; character: CharacterType; oldShirt: string; newShirt: string }[] = [];
	...
	for (const [shirt, nfts] of Object.entries(legendaryShirtNFTs)) {
		...
		for (let i = 1; i < nfts.length; i++) {
			...
			const replacements = isChogstar ? REPLACEMENT_SHIRTS.chogstar : REPLACEMENT_SHIRTS.bearBunnyFox;
			...
			console.log(`  Changing: Token ${nft.tokenId} (${nft.character}) -> ${newShirt}`);
			replacementIndex++;
		}
	}
	...
	for (const change of changes) {
		const metadataPath = join(COMMON_PATH, change.character, "metadata", `${change.tokenId}.json`);
		...
		// Save metadata
		const newMetadataPath = join(OUTPUT_PATH, change.character, "metadata", `${change.tokenId}.json`);
		writeFileSync(newMetadataPath, JSON.stringify(metadata, null, 2));

		// Regenerate image
		const newImagePath = join(OUTPUT_PATH, change.character, "images", `${change.tokenId}.png`);
		await regenerateImage(metadata, change.character, newImagePath);

		console.log(`  Fixed Token ${change.tokenId}: ${change.oldShirt} -> ${change.newShirt}`);
	}

	// Save change log
	writeFileSync(join(OUTPUT_PATH, "fix_log.json"), JSON.stringify(changes, null, 2));
