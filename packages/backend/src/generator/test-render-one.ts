	...
	for (const attr of metadata.attributes) {
		...
		if (!layerPath) {
			...
			for (const folder of legendaryFolders) {
				...
				if (existsSync(testPath)) {
					layerPath = testPath;
					console.log(`Found ${traitType} "${value}" at: legendary/${folder}/${fileName}`);
					break;
				}
			}
		}
		...
		const zIndex = LAYER_ORDER.indexOf(
			...
			layerName === "hand_accessories" ? "hand_accessories" : // Render BEFORE side_hand
			layerName
		);
		layers.push({ path: layerPath, zIndex: zIndex >= 0 ? zIndex : 99, name: traitType });
	}
