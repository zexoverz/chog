...
import { getTraitPath, loadTraitDatabase } from "./traits";

const OUTPUT_PATH = join(
	import.meta.dir,
	"../../output/legendary_accessories_test",
);
const IMAGE_SIZE = 2048;
const SEED = 99999;

// These are now in hand_accessories_gold folder
const LEGENDARY_HAND_ACCESSORIES = [
	"golden_axe.png",
	"golden_lunarstaff.png",
	"golden_sword.png",
	"golden_gun.png",
	"golden_pistol.png",
	"golden_popsicle.png",
	...
	for (const accessory of LEGENDARY_HAND_ACCESSORIES) {
		...
		for (const character of characters) {
			...
			while (!success && attempts < 100) {
				...
				const legendaryHandAcc =
					db.legendary.shared.get("hand_accessories") || [];
				const targetAccessory = legendaryHandAcc.find(
					(t) => t.filename === accessory,
				);
				...
				if (targetAccessory) {
					...
					if (baseTrait) {
						...
						const colorMatch =
							COLOR_MATCHING_COMMON[character]?.[baseTrait.filename];
						if (colorMatch) {
							const handTraits =
								db.common.characters.get(character)?.get("hand") || [];
							const matchedHand = handTraits.find(
...
async function generateImage(
	...
	const imgPath = join(
		OUTPUT_PATH,
		"images",
		`${tokenId}_${character}_${accessoryName}`,
	);
