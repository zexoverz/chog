...
const OUTPUT_PATH = join(import.meta.dir, "../../output/common_redistributed_v2");
...
async function redistributeShirts() {
	...
	for (const char of characters) {
		...
		for (const file of files) {
			...
			if (shirtName !== "None") {
				shirtCountsByGroup[group][shirtName] = (shirtCountsByGroup[group][shirtName] || 0) + 1;
			}
		}
	}
	...
	for (const groupName of ["bearBunnyFox", "chogstar"] as const) {
		...
		const availableShirts = groupName === "chogstar" ? chogstarShirts : bearBunnyFoxShirts;

		console.log(`\n=== Processing ${groupName} group ===`);

		// Filter to only common shirts (not legendary, available in assets)
		const isCommonShirt = (name: string) =>
		...
		for (const shirtName of availableShirts) {
			if (LEGENDARY_SHIRTS.some(l => l.toLowerCase() === shirtName.toLowerCase())) continue;

			const count = shirtCounts[shirtName] || 0;
			if (count < MIN_TARGET) {
				needMore.push({ name: shirtName, current: count, needed: MIN_TARGET - count });
			} else if (count > MIN_TARGET) {
				canGive.push({ name: shirtName, current: count, excess: count - MIN_TARGET });
			}
		}
		...
		console.log(`Shirts needing boost: ${needMore.length} types, total +${totalNeeded}`);
		console.log(`Shirts with excess: ${canGive.length} types, total -${totalExcess}`);

		if (totalExcess < totalNeeded) {
			console.log(`WARNING: Not enough excess in ${groupName}! May not reach target for all.`);
		}
